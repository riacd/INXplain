"""Patched GitEventhandler/H2GCN-PyTorch model.

Source: https://github.com/GitEventhandler/H2GCN-PyTorch/blob/master/model.py

Local fixes:
- use PyTorch native sparse-sparse matmul instead of torch_sparse.spspmm;
- return log_softmax so callers using F.nll_loss receive log-probabilities.
"""
...
class H2GCN(nn.Module):
    ...
    def _indicator(sp_tensor: torch.sparse.Tensor) -> torch.sparse.Tensor:
        ...
        return torch.sparse_coo_tensor(
            indices=csp.indices(),
            values=torch.where(csp.values() > 0, 1, 0).to(csp.values().dtype),
            size=csp.size(),
            dtype=csp.values().dtype,
            device=csp.device,
        )
    ...
    def _spspmm(sp1: torch.sparse.Tensor, sp2: torch.sparse.Tensor) -> torch.sparse.Tensor:
        ...
        return torch.sparse.mm(sp1.coalesce(), sp2.coalesce()).coalesce()

    @classmethod
    def _adj_norm(cls, adj: torch.sparse.Tensor) -> torch.sparse.Tensor:
        n = adj.size(0)
        d_diag = torch.pow(torch.sparse.sum(adj, dim=1).to_dense(), -0.5)
